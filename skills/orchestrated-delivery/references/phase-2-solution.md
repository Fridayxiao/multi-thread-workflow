...
Read this file when researching and comparing solution options.

## Goal

Research current and reusable solutions, verify relevant APIs or practices, and present at least two viable options with tradeoffs.
...
- At least two options are compared, unless the user explicitly approved a single direction.
- Standard and Full mode have at least one completed independent research or design delegate. Lite mode records why delegation was skipped, if skipped.
- Existing or reusable solutions investigated are recorded.
- Freshness and deprecation risks are checked or explicitly recorded.
- The lead agent recommends a default with rationale.
- The user selects one option or approves a combination.

## Rollback

Return to Phase 1 when no option satisfies the confirmed requirements or the user changes the goal.
